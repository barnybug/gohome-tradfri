module github.com/barnybug/gohome-tradfri

go 1.12

require (
	github.com/barnybug/go-cast v0.0.0-20181026143840-5be61902f818 // indirect
	github.com/barnybug/go-tradfri v0.0.0-20190510202758-d0899418a947
	github.com/barnybug/gofsm v0.0.0-20181230212641-1a458869bfdb // indirect
	github.com/barnybug/gogsmmodem v0.0.0-20180823201430-c663385185b4 // indirect
	github.com/barnybug/gohome v0.0.0-20191030171518-f6de0dfad73f
	github.com/bocajim/dtls v0.0.0-20190528173046-23e2f20d5f89 // indirect
	github.com/dustin/go-coap v0.0.0-20190908170653-752e0f79981e // indirect
	github.com/eclipse/paho.mqtt.golang v1.2.0 // indirect
	github.com/eriklupander/dtls v0.0.0-20190304211642-b36018226359 // indirect
	github.com/gogo/protobuf v1.3.0 // indirect
	github.com/google/uuid v1.1.1 // indirect
	github.com/gorilla/context v1.1.1 // indirect
	github.com/gorilla/mux v1.7.3 // indirect
	github.com/gorilla/websocket v1.4.1 // indirect
	github.com/hashicorp/go.net v0.0.1 // indirect
	github.com/hashicorp/mdns v1.0.1 // indirect
	github.com/kurrik/json v0.0.0-20160508230744-6b510c293ed2 // indirect
	github.com/kurrik/twittergo v0.0.0-20190804010542-25ed642452e5 // indirect
	github.com/mattn/go-xmpp v0.0.0-20190124093244-6093f50721ed // indirect
	github.com/mdlayher/apcupsd v0.0.0-20190314144147-eb3dd99a75fe // indirect
	github.com/miekg/dns v1.1.16 // indirect
	github.com/nlopes/slack v0.6.0 // indirect
	github.com/pborman/uuid v1.2.0 // indirect
	github.com/pkg/errors v0.8.1 // indirect
	github.com/stretchr/objx v0.2.0 // indirect
	golang.org/x/crypto v0.0.0-20190907121410-71b5226ff739 // indirect
	golang.org/x/net v0.0.0-20190909003024-a7b16738d86b // indirect
	golang.org/x/sys v0.0.0-20190907184412-d223b2b6db03 // indirect
	golang.org/x/text v0.3.2 // indirect
	golang.org/x/tools v0.0.0-20190909030654-5b82db07426d // indirect
	gopkg.in/check.v1 v1.0.0-20190902080502-41f04d3bba15 // indirect
	gopkg.in/telegram-bot-api.v4 v4.6.4 // indirect
	gopkg.in/yaml.v1 v1.0.0-20140924161607-9f9df34309c0
)
